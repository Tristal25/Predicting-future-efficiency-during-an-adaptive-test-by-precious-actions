...
2. AccessionNumber: A unique identification of a problem/item.

3. ItemType: The type of the item, including:
...
4. Observable: The type of the action the student took.

5. ExtendedInfo: Additional information on the student action (e.g. which choice the student clicked)

6. EventTime: The timestamp of when the action was taken.

7. Time: The length of time since the start of the first action. 

### Data Preprocessing and feature identification

Some pre-processing steps are necessary to make our data clean. In order to evaluate the model that we will use, the data is separated into training set and test set (1000:232). The response variable is regulated into 1 (Efficient) and 0 (Not efficient) for the convenience of regression. 

Since the action data is a large list of data frame, it cannot be fitted into a model easily. Thus, we need to dig into the raw data and identify some important features from the actions in Block A that can affect the efficiency of block B. In other words, going beyond the original format and creating a clean data that is more friendly to modeling. **The details of feature identification here can be seen in Appendix 1: Action data feature identification. **
...
The target variable is binary so this should be a classification problem. A logistic regression can be suitable in this case. However, with a total of 25 features and only 1000 training data, a simple logistic model may result in overfitting (which means that the model performs well on training data but poor on testing data). Consequently, Logistic Lasso or logistic ridge regression can also be candidates of models. 
...
### Model introduction and discussion
...
1. $p_i$: The probability that the ith student can use time efficiently in Block B

2. $\beta_1,...,\beta_n$: coefficients of the model

3. $\beta_0$: intercept of the model

4. $x_1,...,x_{26}$: features in action data for the ith student. They are: numac, VH356862, VH098810, VH098519, VH098808, VH139047, VH098759, VH098740, VH134366, VH098753, VH134387, VH098783, BlockRev, VH098812, VH139196, VH134373, VH098839, VH098597, VH098556, VH098522, VH098779, VH098834, EOSTimeLft, SecTimeOut, HELPMAT8. (Item types are not included for simplicity.)
...
This table below (Table 5) classifies the actions by range of their coefficients. Accession number of a certain action will be classified into a certain range if and only if this action has a coefficient within that range. 
...
This model is accurate enough to be applied to similar adaptive tests with same types of questions and non-problem actions. The prediction can serve as a part of the student academic ability measurement. Students with a predicted high future efficiency of using time will do a more difficult test with less allowed time. At the same time, the range of scores will also be higher. 

## Weaknesses 

Here is a list of several weaknesses of this project:

1. The model we used may be too simple. The efficiency prediction is a convoluted problem and a single logistic ridge regression cannot handle the functional terms in the model. The accuracy of prediction needs improvement. 
...
3. Include students' data collected from other types of adaptive tests, such as language exams, physics tests and essay writing. Use multi-stage cluster sampling. That is, randomly pick a test from each category of tests and randomly choose one academic year. Census can be done for all chosen tests in that academic year. Same analysis as in this project can be done for all the samples. If all the predictions provide good results, we can say that it is possible for all adaptive tests to add efficiency level adjustment. 
